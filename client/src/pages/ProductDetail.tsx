...
export default function ProductDetail({ id }: { id: string }) {
  ...
  return <SiteFrame><Meta title={product.name} description={`${product.name} from the Umaid Craftorium ${product.collection} collection. Request live specifications and material information from the trade desk.`} />
    ...
    {zoomed && <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label={`${product.name} enlarged image`}><button className="gallery-lightbox__backdrop" type="button" aria-label="Close enlarged image" onClick={() => setZoomed(false)} /><div className="gallery-lightbox__content"><img src={gallery[activeImage]} alt={activeImage === 0 ? product.imageAlt : "Related Umaid Craftorium collection imagery"} /><button type="button" className="gallery-lightbox__close" onClick={() => setZoomed(false)}>Close</button></div></div>}
  </SiteFrame>;
}
