...
export type ProjectInquiryItemInput = {
...
export type ProjectInquiryAttachmentInput = {
  storageKey: string;
  filename: string;
  contentType: string;
  byteSize: number;
...
export async function createProjectInquiry(input: {
  ...
  items: ProjectInquiryItemInput[];
  attachments: ProjectInquiryAttachmentInput[];
}) {
  const db = await getDb();
  ...
  return db.transaction(async (tx) => {
    ...
    if (input.attachments.length) {
      await tx.insert(inquiryAttachments).values(input.attachments.map((attachment) => ({ inquiryId, ...attachment })));
    }
    return { id: inquiryId };
  });
}
